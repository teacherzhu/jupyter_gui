"""
JupyterX Notebook extension for Jupyter
...
if ipy_major_version < 4:
    sys.exit("Jupyter X Notebook requires Jupyter 4.2 or greater")
...
@magics_class
class JupyterXMagic(Magics):
    """
    JupyterX Notebook magic commands
    """
...
class GPAuthWidget(GPResource, widgets.DOMWidget):
    """
    A running or completed job on a JupyterX server.
...
def load_ipython_extension(ipython):
    ipython.register_magics(JupyterXMagic)
...
def _jupyter_nbextension_paths():
    return [dict(
        ...
        require="jupyter_x/index")]


def load_jupyter_server_extension(nbapp):
    nbapp.log.info("JupyterX Notebook enabled!")